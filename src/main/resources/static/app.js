...
class SuperBizAgentApp {
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '描述您遇到的问题，例如：开锁失败、计费异常...';
        }
    }
    ...
    async sendAIOpsRequest(loadingMessageElement) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/service_agent`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                }
                                    ...
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'content') {
                                                ...
                                                throw new Error(sseMessage.data || '投诉分析失败');
                                            }
                                        } catch (e) {
    ...
    async triggerAIOps() {
        ...
        const loadingMessage = this.addLoadingMessage('投诉分析中...');
        ...
        try {
            ...
            console.error('投诉分析失败:', error);
            if (loadingMessage) {
                const messageContent = loadingMessage.querySelector('.message-content');
                if (messageContent) {
                    messageContent.textContent = '抱歉，投诉分析时出现错误：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = '智能投诉分析中，请稍候...';
